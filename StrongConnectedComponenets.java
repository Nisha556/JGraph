package Graph;
//strongly connected grpah
import java.util.HashMap;
import java.util.InputMismatchException;
import java.util.Map;
import java.util.Scanner;
import java.util.Stack;
public class StrongConnectedComponenets {
...
public StrongConnectedComponenets(int number_of_nodes)
{
...
public static void main(String args[])
{
	...
	try {
		...
		System.out.println("ENter the node of the adjacency matrix");
		for(int i = 1; i<= number_of_nodes; i++)
			for(int j = 1; j<= number_of_nodes;j++)
				adjacency_matrix[i][j] = scanner.nextInt();
		StrongConnectedComponenets strong = new StrongConnectedComponenets(number_of_nodes);
		strong.strongConnectedComponent(adjacency_matrix);
		System.out.println("the strong connected componenet");
		for(int i = 1; i<strong.leader_node.length; i++)
		{
			System.out.println("Node" +i+"belongs to SSC" + strong.finishing_time_map.get(strong.leader_node[i]));
